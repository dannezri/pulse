...
**Nouveau MVP v2.0** : Corrélation simple entre données de flux (Vital API) et données de contexte (Apple Health) pour générer des insights personnalisés via LLM.

### Architecture Simplifiée

- **Données de Flux** : Vital API (HR, HRV, Sleep) → webhook → `biometrics`
...
### US 1 : En tant qu'utilisateur, je veux que mes données Vital (HR, HRV, Sleep) remontent automatiquement dans Pulse.

**Description** : Webhook Vital API vers backend FastAPI.

**Critères d'acceptation** :
- Vital API envoie un webhook avec `user_id` (Vital) et données (HR, HRV, Sleep)
- Backend mappe `user_id` Vital → `user_id` Supabase via `external_identities`
...
**Note technique** :
- Endpoint : `POST /api/webhooks/vital`
...
- `provider_system` (vital, apple_health, etc.)
...
### Vital Webhook
1. Configurer webhook dans Vital API dashboard
2. URL : `https://your-api.com/api/webhooks/vital`
3. S'assurer que `external_identities` contient les mappings Vital → Supabase
