...
**Nouveau MVP** :
- **Données de Flux** : Vital API (HR, HRV, Sleep) via webhook vers backend
...
│  │  - POST /api/webhooks/vital (HR, HRV, Sleep)            │  │
...
┌─────────────────────────────────────────────────────────────────┐
│                    SOURCES DE DONNÉES                             │
│  Vital API (webhook) │ Apple Health (scan local iOS)            │
└─────────────────────────────────────────────────────────────────┘
```
...
- **`api_server.py`** : API FastAPI
  - `POST /api/webhooks/vital` : Reçoit les données Vital API (HR, HRV, Sleep)
...
- **`supabase_client.py`** : Client Supabase
  - Insertion des `biometrics` (depuis webhook Vital)
...
#### `biometrics`
- Données de flux (HR, HRV, Sleep) depuis Vital API
...
```
1. Vital API envoie webhook (HR, HRV, Sleep)
   POST /api/webhooks/vital
   ↓
...
- **API Keys** : Authentification pour appels API (Vital, OpenAI)
...
2. Adapter `POST /api/webhooks/vital` pour le format du provider
