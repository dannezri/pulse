...
- Backend FastAPI (webhooks Vital, génération insights)
...
- `external_identities` (déjà existante, utilisée pour mapping Vital)
...
### 5. Configurer Webhook Vital

Dans le dashboard Vital API :
1. Aller dans "Webhooks"
2. Ajouter webhook : `https://your-api.com/api/webhooks/vital`
...
VALUES ('supabase-uuid', 'vital', 'vital-user-id', true);
```
...
- [ ] Webhook Vital accepte les payloads valides
...
### Mapping Identity Vital
- Vital `user_id` doit être présent dans `external_identities` avec `provider_system="vital"`
- Si absent : webhook accepté (202) mais non traité
- Créer le mapping lors de la connexion Vital dans l'app

### Idempotence
- Les webhooks Vital utilisent `source_event_id` pour éviter doublons
